...
public class OwnedPokemonController {
    ...
    public ResponseEntity<OwnedPokemonOutputDto> createOwnedPokemon(@RequestBody OwnedPokemonInputDto inputDto) {
        ...
        return ResponseEntity.ok(newOwnedPokemon);
    }
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<String> deleteOwnedPokemon(@PathVariable Long id) {
        String responseMessage = ownedPokemonService.deleteOwnedPokemon(id);
