...
public class PokemonControllerTest {
    ...
    public void testCreatePokemon_ValidPokemon_ReturnsCreated() throws Exception {
        ...
        Mockito.when(pokemonService.savePokemon(Mockito.any(PokemonInputDto.class))).thenReturn(outputDto);
    ...
    @Test
    public void testCreatePokemon_InvalidType_ReturnsBadRequest() throws Exception {
        PokemonInputDto inputDto = new PokemonInputDto();
        inputDto.setName("Clefairy");
        inputDto.setNationalDex(35);
        inputDto.setType(TypeEnum.POKEMON_TYPE.FAIRY);

        Mockito.when(pokemonService.savePokemon(Mockito.any(PokemonInputDto.class)))
                .thenThrow(new IllegalArgumentException("Fairy type is not allowed for generation less than 6."));
